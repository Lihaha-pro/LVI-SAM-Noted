...
public:
    ...
    pcl::PointCloud<pcl::PointXYZRGB> RGB_Cloud;        //RGB渲染之后的点云
    ///image相关
    ...
    RGB() {
        ...
        pubCloudInImage = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/lidar/mapping/cloudInImage", 1);            // localmap的特征点云
    ...
    void imagePoseCallback(const nav_msgs::Odometry::ConstPtr &pose_msg)
    {
        ...
        cv::Mat rawImage = curImg_Time.first;
        m_image.unlock();
        ...
        for (int i = 0; i < pointSize; i++) {
            ...
            cout << "💫相机系下某点坐标为\t" << curPoint.x << "\t\t" << curPoint.y << "\t\t" << curPoint.z << endl;
