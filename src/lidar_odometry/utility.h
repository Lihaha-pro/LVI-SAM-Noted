...
public:
    ...
    int N_SCAN;       // 16线激光雷达, number of lidar channel (i.e., 16, 32, 64, 128)
    int Horizon_SCAN; // lidar horizontal resolution (Velodyne:1800, Ouster:512,1024,2048)
    string timeField;
    int downsampleRate;
    ...
    // extrinsic IMU -> lidar (extTrans, extRot, extQRPY)
    vector<double> extRotV;
    vector<double> extRPYV;
    vector<double> extTransV;
    
    Eigen::Matrix3d extRot; // from extRotV
    Eigen::Matrix3d extRPY; // from extRPYV
    Eigen::Vector3d extTrans; // frome extTransV
    
    Eigen::Quaterniond extQRPY; // from extRPY
