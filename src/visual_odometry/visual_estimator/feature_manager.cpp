...
// 添加特征点, 检测视差 (根据视差决定marge掉最老帧还是次新帧)
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image, double td)
{
    ...
    for (auto &id_pts : image)
    {
        ...
        // 2.在已有的特征点集合里, 寻找该特征点
        // 2.find feature id in the feature bucket
...
// 滑动窗口, 移除features与最老帧的观测关系, 并重新计算features的深度
         ...
         it != feature.end(); it = it_next)
    {
        ...
        else
        {
            ...
            if (it->feature_per_frame.size() == 0)
                feature.erase(it); // 移除观测关系之后, 特征点没有被其他帧观测到
        }
    }
}
...
double FeatureManager::compensatedParallax2(const FeaturePerId &it_per_id, int frame_count)
{
    ...
    const FeaturePerFrame &frame_j = it_per_id.feature_per_frame[frame_count - 1 - it_per_id.start_frame]; // 倒数第三帧
